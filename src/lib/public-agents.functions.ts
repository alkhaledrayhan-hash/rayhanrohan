...
export const listPublicAgents = createServerFn({ method: "GET" }).handler(
  async (): Promise<{ agents: PublicAgent[] }> => {
    ...
    const { data: props, error: cErr } = await supabaseAdmin
      ...
      .or(`assigned_agent_id.in.(${ids.join(",")}),created_by.in.(${ids.join(",")})`)
...
export const getPublicAgent = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      data,
    }): Promise<{ agent: PublicAgentDetail | null; properties: PublicAgentProperty[] }> => {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      ...
      const { data: props, error: pErr } = await supabaseAdmin
        .from("properties")
        .select(
          "id, slug, title, location, address, type, status, price, bedrooms, bathrooms, sqft, image, assigned_agent_id, created_by",
        )
        .or(
          `assigned_agent_id.eq.${data.id},and(assigned_agent_id.is.null,created_by.eq.${data.id})`,
        )
        .eq("listing_status", "approved")
