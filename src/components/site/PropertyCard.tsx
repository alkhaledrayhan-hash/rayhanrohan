...
function GridCard({ property, onQuickView }: { property: Property; onQuickView: (e: React.MouseEvent) => void }) {
  return (
    <Link
      ...
      className="group hover-lift flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)] will-change-transform"
    >
...
function TicketCard({ property, onQuickView }: { property: Property; onQuickView: (e: React.MouseEvent) => void }) {
  return (
    <Link
      to="/properties/$id"
      params={{ id: property.id }}
      className="group hover-lift relative grid overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)] sm:grid-cols-[minmax(0,320px)_1fr]"
    >
      {/* Perforated divider */}
