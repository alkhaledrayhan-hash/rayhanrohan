  ...
  const customImages: string[] =
    Array.isArray(hero.images) && hero.images.length > 0
      ...
      : hero.image_url
        ? [hero.image_url]
        : [];
  const slideInterval: number =
    Number(hero.slide_interval) > 0 ? Number(hero.slide_interval) : 2000;

  const HERO_IMAGES =
    customImages.length > 0 ? customImages : [heroImg, heroImg2, heroImg3, heroImg4];
  ...
  return (
    <section className="relative isolate overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-24 sm:px-6 sm:pt-32 lg:px-8 lg:pb-24 lg:pt-40">
        <div
          className={`max-w-3xl text-white ${heroStyle.align === "center" ? "mx-auto text-center" : ""}`}
        >
          {(hero.eyebrow ?? "Qatar's premium address book") && (
            <span
              ...
              style={
                heroStyle.eyebrow_color
                  ? {
                      color: heroStyle.eyebrow_color,
                      borderColor: withAlpha(heroStyle.eyebrow_color, 60),
                    }
                  : undefined
              }
            >
              <span
                ...
                style={{ background: heroStyle.eyebrow_color || undefined }}
              />
              {hero.eyebrow ?? "Qatar's premium address book"}
            </span>
          )}
          ...
          >
            {hero.subtitle ??
              "Curated residences across Doha, The Pearl, Lusail, West Bay and Al Waab. Rent or buy with a white-glove experience from first viewing to keys in hand."}
          </p>
          {(hero.cta_label || hero.cta2_label) && (
            <div
              className={`mt-6 flex flex-wrap gap-3 ${heroStyle.align === "center" ? "justify-center" : ""}`}
            >
              {hero.cta_label && (
                <Link
                  ...
                  style={{
                    background: heroStyle.cta_bg || "hsl(var(--primary))",
                    color: heroStyle.cta_text || "#ffffff",
                  }}
                >
                  {hero.cta_label}
                </Link>
